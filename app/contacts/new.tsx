...
// ahora incluye proveedor
type ClientType = "productora" | "agencia" | "directo" | "proveedor" | null;
      ...
      for (const tenantId of targets) {
        ...
        if (tenantId) {
          try {
            ...
            console.warn(
              "❌ Error al cambiar de workspace antes de crear contacto:",
              e
            );
          }
        }
        ...
        // 2) Crear nota (si hay) — SIN crear actividad asociada
        if (noteBody) {
          await createNote({
            id: uid(),
            body: noteBody,
        ...
        // ❌ Ya NO se crean actividades automáticas al crear contacto
        // ❌ Ni de tipo note ni de tipo task con recordatorio
      }
    },
    onSuccess: async () => {
      await Promise.all([
...
<View style={styles.clientRow}>
  ...
  <Pressable
    onPress={() =>
      setClientType((prev) => (prev === "proveedor" ? null : "proveedor"))
    }
    style={[
      styles.clientChip,
      clientType === "proveedor" && styles.clientChipActive,
    ]}
  >
    <Text
      style={[
        styles.clientChipText,
        clientType === "proveedor" && styles.clientChipTextActive,
      ]}
    >
      Proveedor
    </Text>
