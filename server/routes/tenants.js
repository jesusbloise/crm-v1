...
/* =========================================================
   GET /tenants → lista workspaces “visibles” para el usuario
   - Admin/Owner GLOBALES: ven todos los workspaces
   - Resto: workspaces que crearon O donde son miembros
========================================================= */
r.get("/tenants", async (req, res) => {
  try {
    const uid = resolveUserId(req);
    const isAdminGlobal = await isAdmin(uid);

    let rows;

    if (isAdminGlobal) {
      // 🔹 Admin global: ve todos los tenants
      rows = db
        .prepare(
          `
          SELECT
      ...
      // 🔹 Usuario normal: los que creó o donde es miembro
      rows = db
        .prepare(
          `
          SELECT DISTINCT
          ...
          LEFT JOIN tenant_memberships tm
            ON tm.tenant_id = t.id AND tm.user_id = ?
